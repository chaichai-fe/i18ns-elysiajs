...
export class AuthService {
    async register(createUserDto: CreateUserDto, signJWT: (payload: any) => Promise<string>): Promise<AuthResponse> {
    ...
    async login(loginDto: LoginUserDto, signJWT: (payload: any) => Promise<string>): Promise<AuthResponse> {
        // Find user
        const [user] = await db
            .select()
            .from(userTable)
